...
//PRODUCT DETAILS
	public function get_product_details(){
		...
		$data['product_price'] = $product_detail['product_market_price'];
	...
	public function quotation(){
		if($this->input->post()){
			...
			$quotation_number = $this->common->get_particular('company_details',array('company_id' => $data['company_id']),'quotation_prefix_value').''.str_pad($this->common->get_particular('company_details',array('company_id' => $data['company_id']),'quotation_prefix_count'),$this->common->get_particular('mst_settings',array('settings_name' => 'estimate_number_of_zeros' ),'settings_value'), '0', STR_PAD_LEFT);
			if($temp_products){
				$quotation = array(
				...
				if(isset($data['quotation_loading_changes'])){
					$quotation['quotation_loading_changes'] = $data['quotation_loading_changes'];
				}
				if(isset($data['quotation_transportaion_changes'])){
					$quotation['quotation_transportaion_changes'] = $data['quotation_transportaion_changes'];
				} 
				if(isset($data['quotation_discount'])){
					$quotation['quotation_discount'] = $data['quotation_discount'];
				}
				...
				if($quotation_id){
					...
					$current_count =$this->common->get_particular('company_details',array('company_id' => $data['company_id']),'quotation_prefix_count');
					$next_count = next_number($current_count);
					$this->common->update('company_details',array('quotation_prefix_count' => $next_count ),array('company_id' => $data['company_id']));
					$quotation_total = 0;
					...
					//DC AUTO GENERATE
					if(($this->common->get_particular('mst_quotation_settings',array('quotation_settings_name' => 'quotation_dc_auto_generate'),'quotation_settings_value'))== 1){
	...
	public function quotation_edit($quotation_id){
		if($this->input->post()){
			...
			$data['company_lists'] 	= convert_options_selected($this->common->gets_array('company_details',array('company_status' =>1,'company_id' => $data['quotation_details']['company_id'])),'company_id','company_name','COMPANY',$data['quotation_details']['company_id']);
			//echo "<pre>";print_r($data);exit;
			$this->template->build('quotation/quotation',$data);
		}
	}
	...
	public function qt_generate_dc($quotation_id){
		...
		if($data){
			$dc = array(
				...
				'quotation_id'			=> $quotation_id,
				'dc_number'				=> $data['quotation_details']['quotation_number'],
				'dc_date'				=> date('Y-m-d'),
